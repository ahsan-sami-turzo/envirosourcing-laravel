    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header card_header">
                <div class="row">
                    <div class="col-md-8">
                        <h4 class="card-title card_title"><i class="fab fa-gg-circle"></i> All Sister Concern Information</h4>
                    </div>
                    <div class="col-md-4 text-right">
                        <a href="{{ route('company.create') }}" class="btn btn-dark btn-md waves-effect btn-label waves-light card_btn"><i class="fas fa-plus-circle label-icon"></i>Add Company</a>
                    </div>
                </div>
            </div>
            <div class="card-body">
                ...
                <table id="alltableinfo" class="table table-bordered table-striped table-hover dt-responsive nowrap custom_table">
                    ...
                    <tbody>
                      @foreach($companies as $data)
                        <tr title="{{$data->company_description ?: ''}}">
                            <td>{{$data->company_name ?: ''}}</td>
                            <td>{{$data->company_title_one ?: ''}}</td>
                            <td><img style="height: 30px" src='{{  asset("uploads/company/$data->company_logo")}}' alt=""></td>
                            <td>
                                <button class="btn btn-sm {{$data->type == 100 ? 'btn-info' : 'btn-danger'}} ">{{$data->type == 100 ? 'Mother company' : 'Sister concern '}}</button>
                            </td>

                            <td>
                                <button class="btn btn-sm {{$data->status == 1 ? 'btn-success' : 'btn-danger'}} ">{{$data->status == 1 ? 'Active' : 'Inactive'}}</button>
...
{{--                                    <a class="dropdown-item" href='#'>Change Permission</a>--}}
                                    <a class="dropdown-item" href='{{ route("company.edit",$data->id) }}'>Edit</a>
