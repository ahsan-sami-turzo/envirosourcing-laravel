    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header card_header">
                <div class="row">
                    <div class="col-md-8">
                        <h4 class="card-title card_title"><i class="fab fa-gg-circle"></i> All Product Information</h4>
                    </div>
                    <div class="col-md-4 text-right">
                        <a href='{{ route("product.create") }}' class="btn btn-dark btn-md waves-effect btn-label waves-light card_btn"><i class="fas fa-plus-circle label-icon"></i>Add Product</a>
                    </div>
                </div>
            </div>
            <div class="card-body">
                ...
                <table id="alltableinfo" class="table table-bordered table-striped table-hover dt-responsive nowrap custom_table">
                    ...
                    <tbody>
                      ...
                      <tr>
                          ...
                          <td>{{ isset($data->company) ? $data->company->company_name : '' }}</td>
                          <td>{{ isset($data->category) ? $data->category->name : '' }}</td>
                          <td> <button class="btn btn-sm {{$data->status == 1 ? 'btn-success' : 'btn-danger'}} ">{{$data->status == 1 ? 'Active' : 'Inactive'}}</button>
                          </td>
                          <td>
                              <img style="height: 30px" src='{{  asset("uploads/product/$data->product_image")}}' alt="">
                          </td>
                          <td>
                            <div class="btn-group">
                              ...
                              <div class="dropdown-menu">
                                   <a class="dropdown-item" href='{{ route("product.edit",$data->id) }}'>Edit</a>
...
<div id="softDelModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <form method="post" action="{{ url('product/softdelete') }}">
      @csrf
