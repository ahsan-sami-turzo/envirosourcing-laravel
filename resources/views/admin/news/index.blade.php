    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header card_header">
                <div class="row">
                    <div class="col-md-8">
                        <h4 class="card-title card_title"><i class="fab fa-gg-circle"></i> All News Information</h4>
                    </div>
                    <div class="col-md-4 text-right">
                        <a href="{{ route('news.create') }}" class="btn btn-dark btn-md waves-effect btn-label waves-light card_btn"><i class="fas fa-plus-circle label-icon"></i>Add News</a>
                    </div>
                </div>
            </div>
            <div class="card-body">
                ...
                <table id="alltableinfo" class="table table-bordered table-striped table-hover dt-responsive nowrap custom_table">
                    ...
                    <tbody>
                      @foreach($news as $data)
                        <tr>
                            <td>{{ $data->title ?: '' }}</td>
                            <td>{!! words( $data->description,20,'...')  !!}</td>
                            <td><img style="height: 30px" src='{{  asset("uploads/news/$data->news_image")}}' alt=""></td>
                            <td>
                                <button class="btn btn-sm {{ $data->type == 101 ? 'btn-info' : 'btn-warning'}} ">{{$data->type == 101 ? 'Industry' : 'Company'}}</button>
                            </td>
...
<div id="softDelModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <form method="post" action="{{url('news/softdelete')}}">
      @csrf
